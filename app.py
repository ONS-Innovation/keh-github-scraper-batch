...
class GitHubDataProducer:
    ...
    def fetch_data(self):
        ...
        logger.info(f"Starting to fetch repositories for organization: {self.org}")
        query = """
        ...
        while self.has_next_page:
            variables = {"org": self.org, "limit": self.batch_size, "cursor": self.cursor}
            ...
            try:
                ...
                logger.debug(f"Added batch of {len(repos)} | Total: {self.repos_fetched}")
        ...
        logger.info(f"Completed fetching repositories. Total repositories fetched: {self.repos_fetched}")
...
class GitHubDataConsumer:
    ...
    def fetch_org_teams(self):
        ...
        try:
            ...
            if "errors" in data:
                logger.error(f"GraphQL query returned errors when fetching organization teams: {data['errors']}")
                return []
            
            teams = []
            if data["data"]["organization"].get("teams") and data["data"]["organization"]["teams"].get("nodes"):
                for team in data["data"]["organization"]["teams"]["nodes"]:
                    teams.append({
                        "name": team.get("name"),
                        "slug": team.get("slug")
                    })
                logger.info(f"Retrieved {len(teams)} teams from organization {self.org}")
            return teams
        except Exception as e:
    ...
    def fetch_codeowners(self, repo_name, default_branch="main"):
        ...
        paths = ["CODEOWNERS", ".github/CODEOWNERS", "docs/CODEOWNERS", ".gitlab/CODEOWNERS"]
        ...
        for path in paths:
            ...
            try:
                ...
                logger.error(f"Error fetching CODEOWNERS from {path} in {repo_name}: {str(e)}")
        ...
        try:
            ...
            if repo["languages"]["edges"]:
                ...
                for edge in repo["languages"]["edges"]:
                    ...
                    stats_dict = self.archived_language_stats if is_archived else self.language_stats
    ...
    def process_data(self):
        ...
        while True:
            ...
            if repos_batch is None:  # Check for termination signal
                logger.info("Received termination signal, no more repositories to process")
                break

            batch_count += 1
            logger.info(f"Processing | Batch: {batch_count} | Repos: {len(repos_batch)}")
        ...
        logger.info(f"Completed | Repos: {self.repos_processed} | CODEOWNERS: {self.codeowners_found}")

def get_repository_technologies(ql, org, batch_size=5):
    """Get technology information for all repositories in an organization using threads"""
    ...
    consumer = GitHubDataConsumer(data_queue, result_queue, ql, org)  # Pass ql and org to consumer
    ...
    archived_private = sum(1 for repo in all_repos if repo["is_archived"] and repo["visibility"] == "PRIVATE")
    archived_public = sum(1 for repo in all_repos if repo["is_archived"] and repo["visibility"] == "PUBLIC")
    archived_internal = sum(1 for repo in all_repos if repo["is_archived"] and repo["visibility"] == "INTERNAL")
    ...
    for lang, stats in consumer.language_stats.items():
        language_averages[lang] = {
            ...
            "average_percentage": round(stats["average_percentage"] / stats["repo_count"], 3),
